...
def universal_downloader_you_get(url: str):
    ...
    if returned_value == 0:  # if the video has successfully been downloaded
        ...
        if (
            ".webm" in new_video_file_name
        ):  # in case the downloaded video has the ".webm" extension, converting it into ".mp4"
            new_video_file_name_without_extension = new_video_file_name.replace(
                ".webm", ""
            )
            command = (
                'ffmpeg -i "'
                + new_video_file_name
                + '" -crf 23 "'
                + new_video_file_name_without_extension
                + '.mp4"'
            )
            returned_value = os.system(command)
...
def youtube_downloader(number_of_unrecognized_urls, url, textFileAsInput, driver):
    ...
    if (
        "Video unavailable" in driver.page_source
    ):  # if the YouTube video is not available anymore
        number_of_unrecognized_urls += 1
        # Printing the error to the console
        print(
            "\n\n⚠️",
            colored(
                "Issue with URL n°{0}".format(number_of_unrecognized_urls).strip(),
                "red",
            )
            + ":\n"
            + url
        ...
        numfiles = sum(
            ...
            if os.path.isfile(os.path.join(DOWNLOAD_DIRECTORY, f)) and f[0] != "."
        )  # cf.: https://arstechnica.com/civis/viewtopic.php?f=20&t=1111535
        ...
        WebDriverWait(driver, 30).until(
            EC.element_to_be_clickable((By.XPATH, "//*[@id='buttons']/a[1]"))
        ).click()
        ...
        while check_crdownload:
            ...
            for f in os.listdir(DOWNLOAD_DIRECTORY):
                if (
                    os.path.isfile(os.path.join(DOWNLOAD_DIRECTORY, f))
                    and f[0] != "."
        ...
        if DOWNLOAD_DIRECTORY != DEFAULT_DOWNLOAD_DIRECTORY:
            ...
            list_of_files_in_Downloads_folder = glob.glob(
                DEFAULT_DOWNLOAD_DIRECTORY + "/*"
            )
            src_file_path = max(list_of_files_in_Downloads_folder, key=os.path.getctime)
            dest_file_path = src_file_path.replace(
...
def instagram_downloader(
    number_of_unrecognized_urls, url, textFileAsInput, driver, project_path
):
    ...
    if dvid.utils.config.firstTimeLoggedInInsta:
        ...
        try:
            ...
            print(
                colored("Warning!\n", "cyan"),
                'Web element "Accept All" button not found. Error message: ',
                e,
            )
        ...
        python_button = WebDriverWait(driver, 5).until(
            EC.presence_of_element_located(
                (
                    By.XPATH,
                    '//*[@id="react-root"]/section/nav/div[2]/div/div/div[3]/div/div/div/div/div[3]/div[1]/a',
                )
            )
        )
        python_button.click()
        ...
        instagram_credentials_text_file = open(
            project_path + "/utils/instagram_credentials.txt"
        )
    ...
    # ---
        ...
        python_field = WebDriverWait(driver, 20).until(
            EC.presence_of_element_located((By.XPATH, '//*[@id="sf_url"]'))
        )
        python_field.send_keys(url)
        ...
        while number_of_retrieved_files < 1:
            ...
            for i in range(len(list_of_available_links)):
                if (
                    list_of_available_links[i].get_attribute("data-type") == "mp4"
                    or list_of_available_links[i].get_attribute("data-type") == "jpg"
                ):
            ...
            elapsed_sec += 1
            print(
                " SAVE_FROM_INSTA still processing since {0}[s]...".format(elapsed_sec)
            )
            if (
                elapsed_sec % 10 == 0
            ):  # while "SAVE_FROM_INSTA" has not been able to process the url during 10[s] intervals, re-click the "Download" button
        ...
        if number_of_videos_to_download > 0:
            ...
            for i in range(len(list_of_available_mp4_links)):
                ...
                print(
                    " downloading video {0}/{1}".format(
                        current_video_number, len(list_of_available_mp4_links)
                    )
                )
...
def facebook_downloader_1(url, driver, project_path):
    ...
    try:
        ...
        # In case we get an error we use a web scraping method
            ...
            print(
                colored("Warning!\n", "cyan"),
                "Current Facebook video could not be downloaded. Error message:\n",
                e,
            )
            try:
                print(
    ...
    # (Other info for downloading private Facebook video, cf.: "Reverse Engineering Facebook API: Private Video Downloader" (https://yasoob.me/2018/04/23/reverse-engineering-facebook-api-private-video-downloader/))
        ...
        if dvid.utils.config.firstTimeFB1Alt:
            ...
            try:
                python_button = WebDriverWait(driver, 10).until(
                    EC.presence_of_element_located(
                        (By.XPATH, '//*[@id="accept-cookie-banner-label"]')
                    )
                )
                python_button.click()
            except Exception as e:
            ...
            sleep(
                7
            )  # works by waiting 5[s], but waiting 7[s] to be sure (web page loading speed depends on internet connection...)
        ...
        source_link = (
            ...
            .until(EC.presence_of_element_located((By.TAG_NAME, "video")))
            .get_attribute("src")
        )
        ...
        try:
            author_name_encoded = driver.find_element_by_xpath(
                "/html/body/div[1]/div/div[4]/div/div[1]/div/div/div/div[1]/header/div[2]/div/div/div[1]/h3/span/strong/a"
            )
        except:
            author_name_encoded = driver.find_element_by_xpath(
                '//*[@id="mobile_injected_video_feed_pagelet"]/div/header/div[2]/div/div/div[1]/h3/span/strong/a'
            )
        author_name = author_name_encoded.text
        try:
            publication_date_encoded = driver.find_element_by_xpath(
                "/html/body/div[1]/div/div[4]/div/div[1]/div/div/div/div[1]/header/div[2]/div/div/div[1]/div/a/abbr"
            )
        except:
            publication_date_encoded = driver.find_element_by_xpath(
                '//*[@id="mobile_injected_video_feed_pagelet"]/div/header/div[2]/div/div/div[1]/div/a/abbr'
            )
...
def facebook_downloader_2(url, driver):
    ...
    python_button = driver.find_element_by_xpath(
        "/html/body/section/div/div/div/div[1]/div/div[5]/div/div/a[2]"
    )
...
def facebook_downloader_3(url, driver):
    ...
    python_button = WebDriverWait(driver, 20).until(
        EC.presence_of_element_located(
            (By.XPATH, "/html/body/div[2]/div/div/div[1]/div/div[2]/div/div[3]/p[1]/a")
        )
    )
    video_link = python_button.get_attribute("href")
    driver.get(video_link)
...
def linkedin_downloader(url, driver):
    ...
    WebDriverWait(driver, 10).until(
        EC.presence_of_element_located(
            (By.XPATH, '//*[@id="login-form"]/div/span/button')
        )
    ).click()
    ...
    source_link = (
        ...
        .until(
            EC.presence_of_element_located(
                (
                    By.CSS_SELECTOR,
                    "#showdata > div:nth-child(2) > div:nth-child(2) > video > source",
                )
            )
        )
        .get_attribute("src")
    ...
    if (
        video_name[-1] == "/"
    ):  # removing the last character of the "video_name" if it is a "/"
        video_name = video_name[:-1]
    number_of_mp4_files_already_in_DOWNLOAD_DIRECTORY = len(
        glob.glob1(DOWNLOAD_DIRECTORY, "*.mp4")
    )
    urllib.request.urlretrieve(
        source_link, DOWNLOAD_DIRECTORY + "/" + video_name + ".mp4"
    )  # downloading the LinkedIn video
...
def dailymotion_downloader(url, driver):
    ...
    video_title_encoded = driver.find_element_by_xpath(
        '//*[@id="sf_result"]/div/div[1]/div[2]/div[1]/div[1]'
    )
...
def zwanzig_minuten_downloader(url, driver):
    ...
    try:
        black_cross_button = WebDriverWait(driver, 2).until(
            EC.presence_of_element_located(
                (By.XPATH, '//*[@id="modal"]/div/div/div/div/div[1]/div[2]')
            )
        )
        ...
        print(
            colored("Warning!\n", "cyan"),
            "Black cross web element button not found. The pop up window might not have shown up. Error message: ",
            e,
        )
    ...
    play_button = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located(
            (
                By.XPATH,
                '//*[@id="__next"]/div/div/div/div/article/section[1]/figure/div',
            )
        )
    )
    play_button.click()
    ...
    while True:
        ...
        # ---
        if any(
            'src="https://unityvideo.appuser.ch/video/' in video_tag_content
            for video_tag_content in video_tags_content_list
        ):
            break
    ...
    start_idx = page_source.find('src="https://unityvideo.appuser.ch/video/') + len(
        'src="'
    )
    ...
    title_encoded = driver.find_element_by_xpath(
        '//*[@id="__next"]/div/div/div/div/article/section[2]/header/h1'
    )
...
def pinterest_downloader(url, driver):
    ...
    for i in range(len(reserved_characters_list)):
        if reserved_characters_list[i] == "\n":
            ...
            translation_dict[reserved_characters_list[i]] = ""
    driver.get(url)
    video_title_encoded = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located(
            (
                By.XPATH,
                '//*[@id="desktopWrapper"]/div[2]/div/div/div[2]/div[1]/div[2]/div[2]/div[2]/div/div[3]/div/h1',
            )
        )
    )
    video_title = video_title_encoded.text
    ...
    source_link = (
        ...
        .until(
            EC.presence_of_element_located(
                (By.XPATH, '//*[@id="showdata"]/div[1]/video/source')
            )
        )
        .get_attribute("src")
    )
