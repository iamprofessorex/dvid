...
def pylint(ctx, loc="local", tests=False, everything=False, specific=""):
    ...
    if tests:
        ...
        ctx.run(
            "pylint --output-format=colorized --rcfile ./lint-configs-python/python/pylintrc tests dvid"
        )
    elif specific:
        ctx.run(
            f"pylint --output-format=colorized --disable=all --enable={specific} --rcfile ./lint-configs-python/python/pylintrc tests dvid"
        )
    else:
        ctx.run(
            "pylint --output-format=colorized --disable=all --enable=F,E --rcfile ./lint-configs-python/python/pylintrc dvid"
        )
...
def mypy(ctx, loc="local", verbose=0):
    ...
    # ctx.run("mypy --config-file ./lint-configs-python/python/mypy.ini dvid tests")
    ctx.run("mypy --config-file ./setup.cfg dvid tests")
...
def isort(ctx, loc="local", check=False, dry_run=False, verbose=0, diff=False):
    """
    isort dvid module. Some of the arguments were taken from the starlette contrib scripts. Tries to align w/ black to prevent having to reformat multiple times.
...
def pre_start(ctx, loc="local", check=True, debug=False):
    ...
    # ctx.run("python dvid/api/tests_pre_start.py")
...
def pytest(
    ...
    _cmd += r" --cov-config=setup.cfg --verbose --cov-append --cov-report=term-missing --cov-report=xml:cov.xml --cov-report=html:htmlcov --cov-report=annotate:cov_annotate  --showlocals --tb=short --cov=dvid tests"
