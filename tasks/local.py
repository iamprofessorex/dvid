...
#     # ctx.run("python ./dvid/api/backend_pre_start.py")
#     # ctx.run("python ./dvid/initial_data.py")
#     # ctx.run("python dvid/dev_serve.py")
...
pip install -r requirements-doc.txt
    ...
    click.secho("[install] install editable version of dvid", fg=COLOR_SUCCESS)
    ctx.run("pip install -e .")
...
def rsync(ctx, loc="local", verbose=0, cleanup=False):
    """
    rsync over files to ~vagrant/dvid folder
    ...
    _cmd = r"""
cd && rsync -r --exclude dvid_venv --exclude .vagrant --exclude .git --exclude .env /srv/vagrant_repos/dvid/ ~/dvid/ && sudo chown vagrant:vagrant -R ~vagrant && cd ~/dvid && ls -lta
    """
...
def pstree(ctx, loc="local", verbose=0, cleanup=False):
    ...
    # lsof -nP +c 15 | grep LISTEN
    _cmd = r"""
ps aux | grep 'python dvid/web.py' | awk '{print $2}' | head -1
    """
