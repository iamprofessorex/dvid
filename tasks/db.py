...
def autogen(ctx, loc="local", verbose=0, clean=False, dry_run=True, comment=""):
    ...
    if clean:
        ...
        # TODO: 2/10/2020 # Add code to remove both the .db file and the files inside of dvid/migrations/*.py
        if (
        ...
        versions = glob.glob("dvid/migrations/versions/*.py")
    ...
    if not comment:
        _crud_models_cmd = r"""
grep "," dvid/api/crud/__init__.py | grep -v "^#" | tr ',' '\n' | xargs
"""
...
"""
    ...
    try:
        if run == "upgrade":
            ...
            _crud_models_cmd = r"""grep "," dvid/api/crud/__init__.py | grep -v "^#" | tr ',' '\n' | xargs
"""
            res = ctx.run(_crud_models_cmd)
            _cmd = r"alembic revision --autogenerate -m 'Initial: {comment}'".format(
                comment=res.stdout.rstrip()
